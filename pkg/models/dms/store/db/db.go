...
import (
	...
	"github.com/lamassuiot/dms-enroller/pkg/models/dms/store"
...
func (db *DB) Insert(ctx context.Context, d dms.DMS) (int, error) {
	...
	err := db.QueryRow(sqlStatement, d.Name, d.SerialNumber, d.KeyType, d.KeyBits, d.CsrBase64, d.Status).Scan(&id)
	span.Finish()
	if err != nil {
...
func (db *DB) SelectAll(ctx context.Context) ([]dms.DMS, error) {
	...
	for rows.Next() {
		var d dms.DMS
		err := rows.Scan(&d.Id, &d.Name, &d.SerialNumber, &d.KeyType, &d.KeyBits, &d.CsrBase64, &d.Status)
		if err != nil {
...
func (db *DB) SelectByID(ctx context.Context, id int) (dms.DMS, error) {
	...
	span.Finish()
	var d dms.DMS
	err := row.Scan(&d.Id, &d.Name, &d.SerialNumber, &d.KeyType, &d.KeyBits, &d.CsrBase64, &d.Status)
	if err != nil {
