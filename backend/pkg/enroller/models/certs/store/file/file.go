...
func (f *File) Insert(id int, data []byte) error {
	...
	if err != nil {
		f.logger.Log("err", err, "msg", "Could not insert certificate in filesystem")
		return err
	}
	defer file.Close()

	if err := pem.Encode(file, &pem.Block{Type: "CERTIFICATE", Bytes: data}); err != nil {
		f.logger.Log("err", err, "msg", "Error encoding bytes as a certificate")
		os.Remove(name)
	}
	return nil
}
...
func (f *File) SelectByID(id int) ([]byte, error) {
	...
	if err != nil {
		f.logger.Log("err", err, "msg", "Could not obtain certificate from filesystem")
		return nil, err
	}
	return data, nil
}
...
func (f *File) Delete(id int) error {
	...
	err := os.Remove(name)
	if err != nil {
		f.logger.Log("err", err, "msg", "Could not delete certificate from filesystem")
		return err
	}
	return nil
}
